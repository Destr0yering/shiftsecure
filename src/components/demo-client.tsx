...
export function DemoClient(){
 const [snapshot,setSnapshot]=useState<RescueSnapshot|null>(null); const [busy,setBusy]=useState(false); const [guideStep,setGuideStep]=useState(0); const [aiResult,setAiResult]=useState<AnalysisResult|null>(null);
 ...
 const analyze=async()=>{setBusy(true);const response=await fetch("/api/ai/decision-summary",{method:"POST",headers:{"x-demo-role":"MANAGER"}});setAiResult(await response.json());setBusy(false)};
 ...
 return <div className="appShell"><header className="topbar"><Link className="brand" href="/"><i>SS</i> ShiftSecure</Link><span className="pill">SIMULATED · PERSISTED SQLITE DEMO</span></header><main className="content">
  ...
  <div className="demoControls">{scenarios.map(([id,label])=><button className={id==="success"?"":"alt"} disabled={busy} key={id} onClick={()=>run(id)}>{label}</button>)}<button className="alt" disabled={busy} onClick={analyze}>Analyze with Gemini</button><button className="alt" disabled={busy} onClick={reset}>Reset</button><a className="primary" href="/api/audit">Export audit JSON</a></div>
  {aiResult&&<section className="aiPanel"><div><span>AI OPERATIONS ANALYSIS</span><b>{aiResult.generatedBy==="GEMINI"?"GEMINI LIVE": "DETERMINISTIC FALLBACK"}</b></div><h3>{aiResult.analysis.operationalRisk} operational risk</h3><p>{aiResult.analysis.situation}</p><p><strong>Human action:</strong> {aiResult.analysis.recommendedHumanAction}</p><small>{aiResult.model} · {aiResult.provider} · {aiResult.latencyMs}ms · confidence {Math.round(aiResult.analysis.confidence*100)}%</small></section>}
