...
export class PravaPaymentProvider implements PaymentProvider{async createAuthorization(input:{amountCents:number;idempotencyKey:string}){void input;return {status:"uncertain" as const,reason:"Live Prava contract and credentials are not verified; mock provider required.",mode:"LIVE" as const}}}
